...
public class OracleDriverTest {
	...
	public static void main(String[] args) {
		...
		//오라클 드라이버를 수동으로 로드(인스턴스 만드는) 메서드=class.forName()
		try {
			Class.forName("oracle.jdbc.driver.OracleDriver");
			System.out.println("ojdbc6드라이버를 정상으로 로드했습니다.");
		} catch (ClassNotFoundException e) {
			//ojdbc6.jar 클래스를 사용할 수 없을 때 에러 발생
			System.out.println("ojdbc6드라이버를 로드 할 수 없습니다.");
		}
		
		//현재 디펜더스로 pom.xml(메이븐), External Jar(직접)등록된 jdbc는 몇 개가 있는지 확인
		//Ex) 개발자가 ojdbc6, mysql, hsql
		...
		//(객체=오브젝트=인스턴스=실행클래스)=> 여기서는 driverList변수
